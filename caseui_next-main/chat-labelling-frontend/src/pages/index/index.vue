    ...
    async getRecommendInfo () {
      try {
        ...
        if (response.data !== null) {
          ...
          const productNames = this.searchedMessage.match(/>([^<]+)(?=<\/a>)/g).map(item => item.slice(1)) // 提取产品名称
          // console.log(matches)
